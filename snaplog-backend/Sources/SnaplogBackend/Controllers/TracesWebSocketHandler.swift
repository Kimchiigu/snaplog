...
final class TracesWebSocketHandler: @unchecked Sendable {
    static let shared = TracesWebSocketHandler()
    ...
    func connect(req: Request, ws: WebSocket) async {
        do {
            ...
            req.logger.notice("trace-ws auth failed: \(error)")
            try? await ws.close()
            return
        }
    ...
    private func subscribe(on app: Application) async {
        ...
        app.redis.subscribe(to: [TraceService.liveChannel]) { _, payload in
