...
final class LogsWebSocketHandler: @unchecked Sendable {
    static let shared = LogsWebSocketHandler()
    ...
    func connect(req: Request, ws: WebSocket) async {
        do {
            ...
            req.logger.notice("netlog-ws auth failed: \(error)")
            try? await ws.close()
            return
        }
    ...
    private func subscribe(on app: Application) async {
        ...
        app.redis.subscribe(to: [NetworkMonitor.netlogChannel]) { _, payload in
